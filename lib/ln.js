...
function ln(name, appenders) {
  ...
  if (Array.isArray(appenders)) {
    ...
    for (var i = 0, n = this.appenders.length; i < n; i++) {
      ...
      if (appender.hasOwnProperty("level") && LEVEL.hasOwnProperty(appender.level)) {
        appender.level = LEVEL[appender.level];
      } else {
      ...
      switch (appender.type) {
        ...
        case "file":
          ...
          var log = function(appender, timestamp, jsonString) {
            var queue = appender.queue;

            if (arguments.length > 1) {
              var path = (appender.isUTC ? moment(timestamp).utc() : moment(timestamp)).format(appender.path);
              queue[path] = queue[path] || [];
              var array = queue[path];
              if (array.length === 0 || array[array.length - 1].length + jsonString.length > PIPE_BUFF) {
                array.push(jsonString);
              } else {
                array[array.length - 1] += jsonString;
              }
            }
...
function log(level) {
  return function() {
    ...
    var jsonString = JSON.stringify(this.fields) + "\n";
    delete this.fields.m;
    delete this.fields.j;

    for (i = 0, n = appenders.length; i < n; i++) {
      appenders[i].emitter.emit("log", appenders[i], timestamp, jsonString);
    }
  };
}
