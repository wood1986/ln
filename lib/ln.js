    ...
    for (var i = 0, n = this.appenders.length; i < n; i++) {
      ...
      switch (appender.type) {
        ...
        case "file":
          ...
          appender.isStaticPath = moment().format(appender.path) === moment().add({ "ms": 111, "s": 1, "m": 1, "h": 12, "d": 1, "w": 1, "M": 3, "y": 1 }).format(appender.path);
          if (appender.isStaticPath) {
            appender.path = moment().format(appender.path);
          }
